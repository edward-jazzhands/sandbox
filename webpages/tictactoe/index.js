...
function checkWinner() {
    ...
    const rows = bitgrid;
    const columns = Array.from({ length: gridSize }, (_value, index) => bitgrid.map(row => row[index]));
...
function checkWinnerWithComments() {
    // return 0 for draw, 1 for player1, 2 for player2
    // return -1 for game not over yet
    const gridSize = bitgrid.length;
    // The l33t chad method of checking for a winner
